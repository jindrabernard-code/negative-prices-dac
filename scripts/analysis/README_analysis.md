...
Implements the calculations from the methodology chapter (ch. 4) plus the descriptive analysis (ch. 3.4) of the accompanying draft (the authoritative source of truth for methodology and interpretation). Input is the merged hourly panel `data/cz_power_panel.csv` (UTC, 2020–2024).

## Structure

| Script | Draft section | What it does |
|---|---|---|
...
```powershell
cd scripts/analysis
python run_all.py            # full pipeline; or e.g. `python run_all.py s1 s4`
```

Outputs go to `scripts/analysis/output/` (`tables/*.csv`, `figures/*.png`, `models/*.npz`). The curated result tables and figures shipped with the repository live in `tables/` and `figures/` at the repository root. Everything is deterministic (seeds in `config.py`).

Dependencies: `pandas`, `numpy`, `scipy`, `statsmodels`, `matplotlib` (see repo `requirements.txt`).

## Methodological notes

- The **estimation window is the post-crisis subperiod** by default (`config.ESTIMATION_PERIOD`), so the surplus regime is not conflated with the 2022 energy-crisis level shift. Change it to run subperiod robustness.
...
- Scenario projections are **conditional statements, not forecasts** — the historical grid-intensity profile and the estimated TVTP link are held fixed outside the observed support (extrapolation risk, discussed in the draft's limitations).
